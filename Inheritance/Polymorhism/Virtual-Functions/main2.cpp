...
include < iostream > #include < string >
    ...
    class Character {
        ...
        public:
            Character(std::string name_, double ID_, double intelegience_, double speed_, double strength_) {
...
class Hero: public Character {
    ...
    public: Hero(std::string name_, double ID_, double intelegience_, double speed_,
        double strength_, int livesSaved_): Character(name_, ID_, intelegience_,
        speed_, strength_) {
...
class Enemy: public Character {
    ...
    public: Enemy(std::string name_, double ID_, double intelegience_, double speed_,
        double strength_, int heroesStopped_,

        std::string type_, bool mainiac_): Character(name_, ID_, intelegience_,
        speed_, strength_) {
...
int main() {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        std::cout << "*** " << humans[i] - > getName() << " ***" << std::endl <<
            ...
            "SPEED : " << humans[i] - > getSpeed() << std::endl << "ID : " << humans[i] - > getID() << std::endl << "STRENGTH : " << humans[i] - > getStrength() <<
            std::endl << "START NOISE : " << humans[i] - > startNoise() << std::endl;
