...
class Character
{
  ...
  public:
    Character(std::string name_, double ID_, double intelegience_, double
      speed_, double strength_)
    {
...
int main()
{
  ...
  for (int i = 0; i < 2; i++)
  {
    ...
    std::cout << "*** " << humans[i]->getName() << " ***" << std::endl << 
      ...
      "SPEED : " << humans[i]->getSpeed() << std::endl << "ID : " << humans[i]
      ->getID() << std::endl << "STRENGTH : " << humans[i]->getStrength() <<
